...
void ESP01_extractADCVals(char* json, uint16_t* adcVals) {
    ...
    // Extract ADC0
    const char* adc0_start = strstr(json, "\"a0\":");
    if (adc0_start) {
        adc0_start += 5; // Move past "\"adc0\":"
        adc0 = atoi(adc0_start);
    }

    // Extract ADC1
    const char* adc1_start = strstr(json, "\"a1\":");
    if (adc1_start) {
        adc1_start += 5; // Move past "\"adc1\":"
        adc1 = atoi(adc1_start);
    }

    // Extract ADC2
    const char* adc2_start = strstr(json, "\"a2\":");
    if (adc2_start) {
        adc2_start += 5; // Move past "\"adc2\":"
...
void ESP01_fetchData(uint16_t* adcVals) {
    ...
    if(ESP01_checkResponse(responseESP, "OK")) {
        ...
        UART0_PrintString(responseESP);
        UART0_PrintString("\r\n");
        ESP01_extractADCVals(responseESP, adcVals);
    }
    else {
